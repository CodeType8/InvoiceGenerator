...
class Editor extends Ext
{
	...
	public function fullJoin($table, $field1, $operator, $field2)
	{
		$this->_fullJoin[] = array(
			"table"    => $table,
			"field1"   => $field1,
			"field2"   => $field2,
			"operator" => $operator
	...
	private function _process($data)
	{
		...
		// 	echo count($data, COUNT_RECURSIVE) . '<br>' . ini_get('max_input_vars');
		// }
	...
	private function _ssp_query($query, $http)
	{
		...
		$this->_perform_full_join($ssp_set_count);
	...
	private function _perform_full_join($query)
	{
		if (count($this->_fullJoin)) {
			for ($i = 0, $ien = count($this->_fullJoin); $i < $ien; $i++) {
				$join = $this->_fullJoin[$i];

				$query->join($join['table'], $join['field1'] . ' ' . $join['operator'] . ' ' . $join['field2'], 'FULL');
			}
		}
	}
	...
	private function _insert_or_update_table($table, $values, $where = null)
	{
		...
		for ($i = 0; $i < count($this->_fields); $i++) {
			...
			// being used)
			if (count($this->_leftJoin) && count($this->_fullJoin) && $tablePart !== $tableAlias) {
				continue;
			}
	...
	{
		if (count($this->_leftJoin) === 0 && count($this->_fullJoin) === 0) {
			return;
		}
